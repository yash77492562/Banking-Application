...
import { userSigninSchema, userLoginSchema, UserSigninSchema, UserLoginSchema } from "@repo/zod/client"; // Adjust the import path as needed

interface SignInProps {
  onSignIn: (data: UserSigninSchema | UserLoginSchema) => void;
}
...
export const SignIn: React.FC<SignInProps> = ({ onSignIn }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const schema = type === "signin" ? userSigninSchema : userLoginSchema;
    const validationResult = schema.safeParse(formData);
    console.log(validationResult)

    if (!validationResult.success) {
      setFormError("Please make sure all fields are filled correctly.");
      return;
    }

    // Call the onSignIn function with the validated formData
    onSignIn(validationResult.data);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="w-5/6 h-5/6 p-12 relative bg-[url('/images/dargonimage.jpg')] bg-cover bg-center bg-no-repeat">
