...
export default function Payer_info() {
    ...
    useEffect(() => {
        if (userId) {
            const decryptUserId = async () => {     
                console.log(userId,'userId from paymentStatus')
                console.log(mode,'mode from paymentStatus')
                try {
                    ...
                    console.log("API response result from paymentStatus:", result);
    ...
    return (
        <div className="w-full h-screen flex flex-col justify-center items-center">
            <PaymentStatus userId={decryptedUserId} mode={decryptedMode} />
        </div>
    );
}
